...
import { Card, CardContent, CardHeader, Checkbox, Collapse, List, ListItem, TextField } from "@mui/material";
...
const ServicePropsCard = ({ serviceKey, serviceModule: { name, authProps, tips } }: ServiceItemProps) => {
  const [serviceProps, setServiceProps] = useRecoilState(servicePropsState(serviceKey))
  if (!serviceProps) {
    return null
  }
  const [tempAuthData, setTempAuthData] = useState<AuthData>()
  ...
  return <ListItem>
    <Card variant="outlined" sx={{ width: "100%" }}>
      <CardHeader title={name} titleTypographyProps={{ variant: "subtitle1" }} action={<>
        <Checkbox checked={serviceProps.enable} onChange={e => updatePartial({ enable: e.target.checked })} />
        {tips && <ExpandMore expand={expanded} onClick={() => setExpanded(!expanded)} />}
      </>} />

      <CardContent>
        {tips && <Collapse in={expanded} timeout="auto" unmountOnExit>
          {tips}
        </Collapse>}
